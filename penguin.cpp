...
void Penguin::move(Sea *seaGrid, Fish fishArr[], KillerWhale *whale1, KillerWhale *whale2)
{
  double distWhale1;
  double distWhale2;
...
void Penguin::moveToFish(Sea *seaGrid, Fish fishArr[], int fishIndex, double distance)
{
  ...
  for (int i = 0; i < loopNum; i++)
  {
    ...
    {
      seaGrid.replace('P', positionX, positionY);
      health += fishArr[fishIndex].getHealth();
      fishArr[fishIndex].setDead(positionX, positionY);
      break;
    }
    else
    {
  ...
  for (int i = 0; i < loopNum; i++)
  {
    ...
    for (int j = 0; j < fishArr.size(); j++)
    {
      if (fishArr[j].getPositionX() == positionX && fishArr[j].getPositionY() == positionY)
      {
        ...
        fishArr[j].setDead(positionX, positionY);
        fishEaten = true;
        break;
      }
    }
